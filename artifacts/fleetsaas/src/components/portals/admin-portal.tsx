...
// 🚀 सिधै रीप्लिट ब्याकइन्डको ठेगाना हार्डकोड गरिएको ताकि भर्सलले सिधै डेटा तानोस्
...
// ── 🚀 थपिएको नयाँ एड्भान्स ह्वाट्सएप मेसेजिङ प्यानल (WhatsApp Alerts) ──
function WhatsAppNotificationsPanel() {
  ...
  // डमी डेटा हिस्ट्री फिड (Scrolling System)
  useEffect(() => {
    setHistory([
      {
        id: 1,
            ...
            {history.map((h) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="truncate text-foreground">{h.target}</p>
                </div>
                <div className="text-right text-[10px] shrink-0 font-medium">
                  <span
                    className={
                      h.status === "success" ? "text-green-500" : "text-red-400"
                    }
                  >
                    {h.status}
                  </span>
                  <p className="text-[9px] text-muted-foreground">{h.time}</p>
                </div>
              </div>
            ))}
          </div>
...
export default function AdminPortal() {
  ...
  return (
    <div className="mx-auto w-full max-w-[860px] p-4 sm:p-6 space-y-6">
      ...
      {adminTab === "fleet-fuel" && (
        <FleetFuelPanel vehicles={vehicles as VehicleRow[] | undefined} />
      )}
      ...
      {adminTab === "fleet-documents" && (
        <FleetDocumentsPanel vehicles={vehicles as VehicleRow[] | undefined} />
      )}
    </div>
  );
}
