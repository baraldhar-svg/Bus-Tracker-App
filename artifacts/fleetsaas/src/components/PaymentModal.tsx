...
const GATEWAYS: { id: Gateway; label: string; color: string; bg: string; border: string; logo: string }[] = [
  {
    id: "esewa",
    label: "eSewa",
...
export default function PaymentModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-background shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            <p className="font-bold text-foreground text-base">Renew Bus Access</p>
            <p className="text-xs text-muted-foreground mt-0.5">Monthly subscription · NPR {AMOUNT.toLocaleString()}</p>
          </div>
          {phase !== "processing" && phase !== "success" && (
            <button onClick={onClose} className="rounded-xl p-2 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
              <X size={16} />
            </button>
          )}
        </div>
        ...
        {phase === "select" && (
          <div className="p-5 space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-4">Choose Payment Method</p>
            {GATEWAYS.map((g) => (
              <button
                key={g.id}
                ...
                <div className="h-10 w-14 rounded-lg bg-white flex items-center justify-center border border-border overflow-hidden shrink-0">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      (e.target as HTMLImageElement).parentElement!.innerHTML = `<span class="text-xs font-bold">${g.label}</span>`;
                    }}
                  />
                </div>
                <div className="flex-1">
                  <p className={`font-bold text-sm ${g.color}`}>{g.label}</p>
                  <p className="text-xs text-muted-foreground">Pay NPR {AMOUNT.toLocaleString()}</p>
                </div>
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="rounded-xl border border-border bg-muted/40 p-4 space-y-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-semibold text-foreground">Bus Access · 30 days</span>
              </div>
              <div className="h-px bg-border" />
              <div className="flex justify-between items-center">
                <span className="text-sm font-bold text-foreground">Total</span>
                <span className="text-lg font-bold text-foreground">NPR {AMOUNT.toLocaleString()}</span>
              </div>
            </div>
            ...
            {phase === "processing" && (
              <div className="flex flex-col items-center gap-3 py-4">
                <div className={`h-14 w-14 rounded-full flex items-center justify-center ${gw.bg} ${gw.border} border-2`}>
                  <Loader2 size={24} className={`${gw.color} animate-spin`} />
                </div>
                <p className="text-sm font-semibold text-foreground">Processing payment…</p>
                <p className="text-xs text-muted-foreground">Please wait, verifying with {gw.label}</p>
              </div>
            )}
            ...
            {phase === "success" && (
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <p className="text-xs text-muted-foreground text-center">Bus access renewed for 30 days via {gw.label}</p>
              </div>
            )}
            ...
            {phase === "confirm" && (
              <div className="flex gap-2">
                <button
                  onClick={() => { setPhase("select"); setGateway(null); }}
                ...
                <button
                  ...
                  className={`flex-1 rounded-xl py-2.5 text-xs font-bold text-white transition-colors ${
                    gw.id === "esewa" ? "bg-green-600 hover:bg-green-700" :
                    gw.id === "khalti" ? "bg-purple-600 hover:bg-purple-700" :
